...
- 送信者のアドレスに表示名が含まれており、表示名にコンマが含まれている場合は、表示名を引用符で囲む必要があります。例えば：
...
- 無効です。表示名と左山かっこの間にスペースはありません。
    
  ```
  From: Office 365<sender@contoso.com>
  ```

- 無効です。[表示名] と [左山かっこ] の前後には、二重引用符の間にスペースを入れることはできません。
