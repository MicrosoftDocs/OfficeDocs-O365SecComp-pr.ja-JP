...
DLP ポリシーで Windows Server fci プロパティまたはその他のプロパティを使用するには、その前に SharePoint 管理センターで管理プロパティを作成する必要があります。理由は次のとおりです。
...
最初に、DLP ポリシーで参照するプロパティを使用してドキュメントをアップロードする必要があります。Office 365 はプロパティを検出し、クロールされたプロパティを自動的に作成します。次の手順では、管理プロパティを作成し、管理プロパティをこのクロールされたプロパティにマップします。
  
### <a name="step-2-create-a-managed-property"></a>手順 2: 管理プロパティを作成する

1. Office 365 管理センターにサインインします。
    
2. 左側のナビゲーションで、[**管理センター** \> **SharePoint**] を選択します。これで、SharePoint 管理センターが表示されます。
...
1つのルールは、個人を**特定できる情報**のプロパティが**高**または中**程度**であるコンテンツへのアクセスをブロックします。2番目のルールは、**個人を特定できる情報**のプロパティが [低] と**なっ**ているコンテンツに関する通知を送信します。
...
> [!CAUTION]
> サイトを再インデックス化すると、検索システムで大量の負荷が発生する可能性があります。シナリオで絶対に必要でない限り、サイトを再インデックス化しないでください。 
