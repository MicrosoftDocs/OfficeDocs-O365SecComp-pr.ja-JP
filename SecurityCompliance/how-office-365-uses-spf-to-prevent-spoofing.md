...
IP アドレス #12 は contoso.com の SPF TXT レコードにないため、メッセージは SPF チェックに失敗し、受信者はそのメッセージをスパムとしてマークすることを選択できます。
  
### <a name="example-3-spf-and-forwarded-messages"></a>例 3:SPF と転送されたメッセージ
<a name="spfExample3"> </a>

SPF の欠点の 1 つは、メールが転送されたときには機能しないことです。たとえば、woodgrovebank.com のユーザーが、outlook.com アカウントにすべての電子メールを送信する転送ルールを設定しているとします。
  
![電子メール メッセージが転送される際に SPF がメールを認証できないことを示す図。](media/6e92acd6-463e-4a1b-8327-fb1cf861f356.jpg)
  
メッセージは当初は、woodgrovebank.com で SPF チェックに合格しますが、IP #25 が contoso.com の SPF TXT レコードにないため、outlook.com では SPF チェックが失敗します。このため、Outlook.com はメッセージをスパムとしてマークする可能性があります。この問題を回避するには、SPF を DKIM や DMARC などの他の電子メールの認証方法と組み合わせて使用します。
...
完全にホストされている Office 365 のユーザーである場合 (つまり、送信メールを送信するオンプレミスのメール サーバーを使用していない)、これが Office 365 用に発行する必要のある唯一の SPF TXT レコードになります。
  
ハイブリッド展開を構成している (つまり、メールボックスの一部はオンプレミスであり、他は Office 365 でホストされている) 場合や、スタンドアロンの Exchange Online Protection (EOP) を利用しているユーザーである (つまり、組織で EOP を使用してオンプレミスのメールボックスを保護している) 場合には、オンプレミスの各エッジ メール サーバーの送信 IP アドレスを DNS の SPF TXT レコードに追加する必要があります。
...
- 通常、強制ルールは次のいずれかです。
  ...
  - ~all
    ...
    soft fail を示します。一覧に IP アドレスのすべてが網羅されているかどうかがわからない場合は、~all (soft fail) 修飾子を使用してください。また、p=quarantine または p=reject と共に DMARC を使用している場合も、~all を使用できます。それ以外の場合は、-all を使用します。
...
複数の送信メール サーバーがある場合は、各メール サーバーの IP アドレスを、スペースの後に "ip4:" ステートメントを付けて区切り、SPF TXT レコードに追加します。たとえば、次のようにします。
...
電子メール メッセージが配信される前に 10 を超える DNS 参照が行われると、受信側のメール サーバーが  _permerror_ とも呼ばれる永続的なエラーで応答するため、メッセージが SPF チェックに失敗する原因となります。また、受信側のサーバーは、次のようなエラーを含む配信不能レポート (NDR) で応答することもあります。
  
- メッセージのホップ数を超えました。
    
- メッセージに必要な参照数が多すぎます。
    
## <a name="avoiding-the-too-many-lookups-error-when-you-use-third-party-domains-with-office-365"></a>Office 365 でサード パーティのドメインを使用する場合の "参照が多すぎます" エラーを回避する
<a name="SPFTroubleshoot"> </a>
