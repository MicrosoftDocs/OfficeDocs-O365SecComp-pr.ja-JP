...
> [!NOTE]
> <sup>\*</sup>アーカイブ先のメールボックスの最初の記憶域のクォータでは、100 GB を Exchange Online (プラン 2) のライセンスを持つユーザーです。ただし、アーカイブの自動拡張を有効にすると、ユーザーのアーカイブとアーカイブの回復可能なアイテム フォルダーの記憶域のクォータ制限はありません。自動拡張の詳細については、 [Office 365 で無制限のアーカイブの概要](unlimited-archiving.md)を参照してください。 
