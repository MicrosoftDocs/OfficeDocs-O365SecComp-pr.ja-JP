...
- セキュリティ/コンプライアンスセンターの電子情報開示アクティビティ
...
- Microsoft PowerApps でのユーザーおよび管理者のアクティビティ
...
- Office 365 監査ログの検索を開始する前に、まず監査ログを有効にする必要があります (または別の管理者)。 これをオンにするには、Security & コンプライアンスセンターの [**監査ログの検索**] ページで [**ユーザーと管理者のアクティビティの記録を開始**する] をクリックします。 (このリンクが表示されていない場合は、組織の監査が既に有効になっています。)オンにすると、監査ログが準備されていて、準備が完了してから数時間で検索を実行できることを示すメッセージが表示されます。 これを行う必要があるのは1回だけです。 
    
    > [!NOTE]
    > 既定では、監査を有効にするプロセスが進行中です。 それまでは、前述のように有効にすることができます。 
  
- Office 365 監査ログを検索するには、Exchange Online で [表示のみの監査ログまたは監査ログの役割が割り当てられている必要があります。 既定では、これらの役割は、Exchange 管理センターの [**アクセス許可**] ページで、コンプライアンス管理および組織の管理役割グループに割り当てられます。 Office 365 および Microsoft 365 の全体管理者は、Exchange Online で組織の管理役割グループのメンバーとして自動的に追加されることに注意してください。 ユーザーが Office 365 監査ログを最低限の特権レベルで検索できるようにするには、Exchange Online でカスタム役割グループを作成し、表示のみの監査ログまたは監査ログの役割を追加し、そのユーザーを新しい役割グループのメンバーとして追加することができます。 詳細については、「 [Manage role groups in Exchange Online](https://go.microsoft.com/fwlink/p/?LinkID=730688)」を参照してください。
...
- 前述したように、監査ログの検索に使用される基礎となるコマンドレットは、Exchange Online コマンドレットで、 **search-unifiedauditlog**です。 これは、このコマンドレットを使用して、Security & コンプライアンスセンターの [**監査ログの検索**] ページを使用する代わりに、Office 365 監査ログを検索できることを意味します。 このコマンドレットは、Exchange Online 組織に接続されたリモート PowerShell で実行する必要があります。 詳細については、「 [search-unifiedauditlog](https://go.microsoft.com/fwlink/p/?linkid=834776)」を参照してください。
...
- イベントが発生してから対応する監査ログ エントリが検索結果に表示されるまで最長 30 分または最長 24 時間かかる場合があります。次の表は、Office 365 のさまざまなサービスで、この処理に要する時間を示しています。
    ...
    |セキュリティ センターとコンプライアンス センター  <br/> |![チェック マーク](media/f3b4c351-17d9-42d9-8540-e48e01779b31.png)           <br/> ||
...
- azure Active Directory (azure AD) は、Office 365 のディレクトリサービスです。 統合監査ログには、Microsoft 365 管理センターまたは Azure 管理ポータルで実行されたユーザー、グループ、アプリケーション、ドメイン、およびディレクトリアクティビティが含まれています。 azure AD イベントの完全な一覧については、「 [azure Active Directory 監査レポートイベント](https://go.microsoft.com/fwlink/p/?LinkID=616549)」を参照してください。
...
3. セキュリティ & コンプライアンスセンターの左側のウィンドウで、[**検索**] をクリックし、[**監査ログの検索**] をクリックします。
...
このセクションの表では、Office 365 で監査されるアクティビティについて説明します。 これらのイベントを検索するには、セキュリティ/コンプライアンスセンターで監査ログを検索します。
...
|サイト コレクション管理者の追加  <br/> |SiteCollectionAdminAdded  <br/> |サイトコレクションの管理者または所有者は、サイトのサイトコレクション管理者として個人を追加します。 サイト コレクション管理者は、サイト コレクションとすべてのサブサイトに対してフル コントロールのアクセス許可を持っています。 このアクティビティは、管理者がユーザーの OneDrive アカウントにアクセスできる場合にも記録されます (SharePoint 管理センターでユーザープロファイルを編集するか[、Microsoft 365 管理センターを使用し](https://docs.microsoft.com/office365/admin/add-users/get-access-to-and-back-up-a-former-user-s-data#part-1---get-access-to-the-former-employees-onedrive-for-business-documents)ます)。 <br/> |
...
|Sway の外部共有の無効化  <br/> |SwayExternalSharingOff  <br/> |管理者は、Microsoft 365 管理センターを使用して、組織全体の外部 Sway 共有を無効にします。  <br/> |
|Sway の外部共有の有効化  <br/> |SwayExternalSharingOn  <br/> |管理者は、Microsoft 365 管理センターを使用して、組織全体の外部 Sway 共有を有効にします。  <br/> |
|Sway のサービスの無効化  <br/> |SwayServiceOff  <br/> |管理者は、Microsoft 365 管理センターを使用して、組織全体で Sway を無効にします。  <br/> |
|Sway のサービスの有効化  <br/> |SwayServiceOn  <br/> |管理者は、Microsoft 365 管理センター (sway サービスは既定で有効になっています) を使用して、組織全体で sway を有効にします。  <br/> |
...
次の表に、管理者が Microsoft 365 管理センターまたは Azure 管理ポータルを使用してユーザーアカウントを追加または変更したときに記録されるユーザー管理アクティビティを示します。
...
次の表に、管理者またはユーザーが Office 365 グループを作成または変更したとき、または管理者が Microsoft 365 管理センターまたは Azure 管理ポータルを使用してセキュリティグループを作成したときに記録されるグループ管理アクティビティの一覧を示します。 Office 365 のグループの詳細については、「 [Microsoft 365 管理センターのグループの表示、作成、および削除](https://support.office.com/article/a6360120-2fc4-46af-b105-6a04dc5461c7)」を参照してください。
...
次の表に、管理者が Microsoft 365 管理センターまたは Azure 管理ポータルで管理者の役割を管理したときに記録される Azure AD の役割管理アクティビティの一覧を示します。
...
次の表に、管理者が Microsoft 365 管理センターまたは Azure 管理ポータルで Office 365 組織を管理したときに記録される Azure AD ディレクトリおよびドメイン関連アクティビティを示します。
...
セキュリティ/コンプライアンスセンターまたは対応する PowerShell コマンドレットを実行することによって実行されるコンテンツ検索と電子情報開示関連のアクティビティは、監査ログに記録されます。 これには、次の操作が含まれます。
...
Microsoft Flow のアクティビティについては、監査ログを検索できます。 これらのアクティビティには、フローの作成、編集、削除、およびフローのアクセス許可の変更が含まれます。 フローアクティビティの監査の詳細については、「 [Security & コンプライアンスセンターで利用可能になった Microsoft Flow audit audit events](https://flow.microsoft.com/blog/security-and-compliance-center)」を参照してください。
...
**現在監査されている別の Office 365 サービスとは何ですか。**

Exchange online、SharePoint Online、OneDrive for business、Azure Active Directory、Microsoft Teams、Dynamics 365、Advanced Threat Protection、Power BI など、最もよく使用される Office 365 サービスが監査されます。 監査対象のサービスの一覧については、[この記事の冒頭](search-the-audit-log-in-security-and-compliance.md)を参照してください。
...
**セキュリティ/コンプライアンスセンターまたは Office 365 Management Activity API を使用する以外に、監査ログを取得する方法はありますか。**

いいえ。 Office 365 監査サービスからデータを取得するには、次の2つの方法しかありません。 
...
いいえ。 監査サービスパイプラインはリアルタイムに近いため、重複除外をサポートできません。
