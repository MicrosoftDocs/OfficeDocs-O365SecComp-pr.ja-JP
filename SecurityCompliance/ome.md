...
この資料は、Office 365 のメッセージの暗号化についての記事の大規模な一連の一部です。必要な情報をすばやく検索するのにには、次の表を使用します。
  
|||
|:-----|:-----|
|この記事を読む.  <br/> |使用する場合.  <br/> |
|[Office 365 で保護されたメッセージについてください。](https://support.office.com/article/2baf3ac7-12db-40a4-8af7-1852204b4b67.aspx) <br/> |暗号化されたメッセージの作業の方法について詳しく学習する必要があるエンド ・ ユーザーと、どのようなオプションが利用できます。  <br/> |
|[保護されたメッセージを開く方法ですか。](https://support.office.com/article/1157a286-8ecc-4b1e-ac43-2a608fbf3098.aspx) <br/> |エンド ・ ユーザーに送信された保護されたメッセージを読み取る必要があります。この資料には、いくつかのバージョンの Outlook と Office 365 以外で gmail など yahoo! アカウントなど、別の電子メール アカウントからメッセージを読み取る方法に関する情報が含まれています。  <br/> |
|[送信、表示、および Outlook で暗号化されたメッセージに返信します。](https://support.office.com/article/eaa43495-9bbb-4fca-922a-df90dee51980.aspx) <br/> |エンド ・ ユーザーに送信、表示、または Outlook で暗号化されたメッセージに返信しようとしています。されていない場合、Office 365 の組織のメンバー、暗号化されたメッセージを Outlook で送信の通知も表示されます。方法については、表示し、Office 365 から送信された暗号化メッセージに返信するには、この資料を使用します。  <br/> |
|[デジタル署名または暗号化されたメッセージを送信します。](https://support.office.com/article/a18ecf7f-a7ac-4edd-b02e-687b05eff547) <br/> |エンド ・ ユーザーが送信、表示、または Outlook for mac を使用して、暗号化されたメッセージに返信するにはこの資料では、S/MIME などのホーム以外の暗号化方法の使用についても説明します。  <br/> |
...
Exchange Online の暗号化メールの流れの規則に一致する電子メール メッセージを送信ユーザーと送信される前にメッセージが暗号化されます。メールを Outlook クライアントを使用するすべての Office 365 ユーザーには、ネイティブな送信者と同じ組織内に含まれていない場合でも、暗号化および権利が保護されたメールの経験を読む最初のクラスが表示されます。Outlook デスクトップで、Outlook の Mac、iOS および Android、モバイルの Outlook と Outlook Web App がサポートされている Outlook クライアントに含まれます。
  
Outlook.com、Gmail、yahoo アカウントに送信された暗号化、または権利が保護されたのメールを受信する暗号化されたメッセージの受信者は、自分の Microsoft アカウントまたは Gmail や yahoo の資格情報を使用してホーム ポータルに簡単に認証できます。
  
権利で保護された、または暗号化されたメールを Outlook 以外のクライアントであるエンド ・ ユーザーは、受信した暗号化および権利が保護されたメッセージを表示するのにはホームのポータルを使用します。
...
メール フロー ルールは、柔軟性があり、1 つの規則で特定のセキュリティ要件を満たすことができますので、条件を結合すること。たとえば、指定されたキーワードが含まれているし、外部の受信者にアドレス指定されるすべてのメッセージを暗号化するためにルールを作成できます。Office 365 のメッセージの暗号化のための新しい機能では、暗号化された電子メールの受信者からの返信も暗号化します。
...
Office 365 の組織では、暗号化されたメッセージの受信者は PC の Outlook、Outlook for Mac、web 上で Outlook、iOS は、Outlook Android は、Outlook など、Outlook の任意のバージョンにシームレスにそれらのメッセージを読み取ることになります。他の電子メール クライアントで暗号化されたメッセージを受信するユーザーは、ホーム ・ ポータル内のメッセージを表示できます。
