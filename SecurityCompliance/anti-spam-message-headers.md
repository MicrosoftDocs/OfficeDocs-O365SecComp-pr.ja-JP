...
|SFV:SKS|コンテンツフィルターによって処理される前に、メッセージがスパムとしてマークされました。これには、メッセージがメールフロールール (トランスポートルールとも呼ばれます) と一致して、自動的にスパムとしてマークされ、追加のフィルター処理をすべてバイパスするメッセージが含まれます。|
...
|SFV:SKN|コンテンツフィルターによって処理される前に、メッセージが非スパムとしてマークされました。これには、メッセージが自動的に非スパムとしてマークし、他のすべてのフィルター処理をバイパスするメールフロールールと一致したメッセージが含まれます。|
