 ...
 **rest のデータの例**としては、SharePoint ライブラリにアップロードされたファイル、Project Online のデータ、Skype for business の会議にアップロードされたドキュメント、電子メールメッセージ、および Office のフォルダーに保存されている添付ファイルなどがあります。メールボックスおよび OneDrive for business にアップロードされたファイル。 
