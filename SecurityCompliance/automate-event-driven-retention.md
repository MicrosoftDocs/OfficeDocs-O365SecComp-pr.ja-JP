...
イベントによって保持時間が開始され、対象種類のイベントに適用されるラベルが付いたすべてのコンテンツに、そのラベルの保持アクションが実行されます。これを、イベント ベースの保持と呼びます。詳しくは、[イベント適応型保持の概要](event-driven-retention.md)をご覧ください。
...
1. コンプライアンス管理者がイベントタイプの作成を行います - 例えば、従業員の解雇、契約の有効期限、製品の製造終了など（[イベント適応型保持](https://docs.microsoft.com/ja-JP/office365/securitycompliance/event-driven-retention)の段階的プロセスを参照してください）
    
1. コンプライアンス管理者は、業務内容に基づいて保持ラベルを作成し、そのラベルをイベントタイプに関連付けます。
...
- **管理センターのUIを使用する**これは、一度に保存するコンテンツを少なくするために使用できるプロセスで、保持をトリガーする頻度は、月ごと、年ごとなど、あまりありません。この方法について詳しくは、[イベント適応型保持の概要](event-driven-retention.md)を参照してください。ただし、この方法で保持をトリガーすると、時間がかかり、エラーが発生しやすくなるため、拡張性が妨げられます。したがって、保持をトリガーするための自動化された一枚岩のソリューションは、データのセキュリティとコンプライアンスを強化することができます。
...
![従業員が組織のメンバーでなくなるシナリオに関する役割とアクションの図](media/automate-event-driven-retention-employee-termination-diagram.png)
  ...
  - セキュリティ＆コンプライアンスセンターにログインします。

  - SCC Adminは、「従業員解雇」、「従業員雇用」などの従業員関連のイベントタイプを作成します。

  - SCC Adminは、「従業員保持」ラベルを作成します。
...
![契約の有効期限シナリオの役割とタスクの図](media/automate-event-driven-retention-contract-expiration.png)
  ...
  - SCC Adminは、 「契約締結」、 「契約解消」イベントなどの契約関連イベントタイプを作成します。

  - SCC Adminは「契約解消」ラベルを作成します。
...
![製品のライフサイクル シナリオの役割とタスクの図](media/automate-event-driven-retention-product-lifecycle-expiration.png)
  ...
  - SCC Adminは、「製造開始」、「製造終了」などの従業員関連のイベントタイプを作成します。

  - SCC Adminは「製造終了」ラベルを作成します。
